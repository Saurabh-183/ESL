...
// import EditGates from '@/components/dialogs/edit-gates'
...
const ListCounter: React.FC = () => {
  ...
  const fetchCounterList = async () => {
    try {
      const response = await fetch(`${API_URL}/admin/counter-list?locationId=${selectedlocation.id}`, {
        headers: {
  ...
  const columns = useMemo<ColumnDef<DataType, any>[]>(
    () => [
      columnHelper.accessor('counterCode', {
      ...
      columnHelper.accessor('id', {
        cell: info => (
          <svg
            ...
            viewBox='0 0 512 512'
            onClick={() => {
              router.push(`/counter/create-counter/${info.getValue()}`)
              // setId(info.getValue())
              // const data = JSON.parse(userdetails)
              // const arr = data.permission?.filter((ele: any) => ele.permissionId === 8)
      ...
      <div className='flex'>
        ...
        <Grid item xs={12} sm={4} className='px-6 pb-4'>
          <CustomAutocomplete
            ...
            renderInput={params => <CustomTextField placeholder='Select Location' {...params} label='Location' />}
          />
        </Grid>
      </div>
