...
// type FormLocation = FormLocationValues & FormId
...
const ListUserPermission: React.FC = () => {
  // States
  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([]);
  const [globalFilter, setGlobalFilter] = useState("");
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  // const [RolePermissionData, setRolePermission] = useState<DataType[]>([])

  // const [selectedroleId, setselectedroleId] = useState<any>(null)
  // const [selectedrolePermissionId, setselectedrolePermissionId] = useState<any>(null)
  ...
  //   try {
  //     const response = await fetch(`${API_URL}/admin/list-role-permission?roleId=${selectedroleId.id}`, {
  //       headers: {
  //         Accept: 'application/json',
  //         Authorization: `Bearer ${token}`
  //       }
  ...
  //       const data = await response.json()
  //       const newData = data.map((ele: any) => {
  //         return { id: ele.permissionId, permissionName: ele.permissionName }
  //       })
  //       if (selectedrolePermissionId !== null) {
  //         setselectedrolePermissionId(null)
  //       }
  //       setRolePermission(newData)
  //     } else {
    ...
    <Card>
      ...
      <div className="flex">
        ...
        <Grid item xs={12} sm={4} className='px-6 pb-4 '>
          <CustomAutocomplete
            fullWidth
            options={RolePermissionData}
            id='autocomplete-controlled'
            value={selectedrolePermissionId}
            onChange={(e: any, newvalue: any) => {
              setselectedrolePermissionId(newvalue)
            }}
            isOptionEqualToValue={(option, value) => option.id === value.id}
            getOptionLabel={(option: any) => option.permissionName || ''}
            renderInput={params => <CustomTextField placeholder='Select Permission' {...params} label='Permission' />}
          />
        </Grid> */}
      </div>
      <div className="overflow-x-auto">
        ...
        {/* <EditGates open={open} setOpen={setOpen} data={gatedata} API_URL={API_URL} UpdateGates={UpdateGates} /> */}
      </div>
      <TablePagination
