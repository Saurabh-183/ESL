...
export default function DataFieldsPage() {
  ...
  const handleMoveRow = (i: number, idx: number, dir: number) => setFieldData((prev) => {
    const c = [...prev];
    const arr = [...c[i]];
    const t = arr[idx + dir];
    if (t) {
      ...
      <div className="flex space-x-4 border-b mb-6">
        {TABS.map((t) => (
          <button
            ...
            className={`px-4 py-2 rounded-xl hover:cursor-pointer hover:bg-gray-300 ${
              activeTab === t
                      ...
                      {fieldData[i].map((row, idx) => (
                        <tr key={idx} className="border-b">
                          ...
                          <td className="flex gap-4 p-2">
                            <button onClick={() => handleMoveRow(i, idx, -1)} className="text-blue-500 ml-2">↑</button>
                            <button onClick={() => handleMoveRow(i, idx, 1)} className="text-blue-500 ml-2">↓</button>
                            <button onClick={() => handleDeleteRow(i, idx)} className="text-red-500 ml-2">
                              <Trash2 size={16} />
                            </button>
                          </td>
                        </tr>
